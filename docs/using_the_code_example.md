...
The following example clones the "[USB-Suspend-Resume](https://github.com/Infineon/mtb-example-psoc-edge-usb-suspend-resume)" application with the desired name "USBSuspendResume" configured for the *KIT_PSE84_EVAL_EPC2* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSE84_EVAL_EPC2 --app-id mtb-example-psoc-edge-usb-suspend-resume --user-app-name USBSuspendResume --target-dir "C:/mtb_projects"
   ```
